...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Bitcoin network. This means one can have an always-on bitcoinIId instance running, and another local bitcoinIId/bitcoincashII-qt instance which connects to this node and receives blocks from it.

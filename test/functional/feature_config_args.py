...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in bitcoincashII.conf or in an included conf file
        ...
        with open(self.nodes[0].datadir_path / 'bitcoincashII.conf', 'a', encoding='utf-8') as conf:
    ...
    def test_config_file_log(self):
        ...
        # Write a bitcoincashII.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.bitcoinIIconf.read_text()
        conf_path = default_datadir / "bitcoincashII.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a bitcoincashII.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitcoincashII.conf" file which is ignored, because a '
        ...
        # the same bitcoincashII.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/bitcoincashII.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when bitcoincashII.conf in the default data directory sets another datadir '
                      'and it contains a different bitcoincashII.conf file that would be ignored')
        ...
        # bitcoincashII.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "bitcoincashII.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitcoincashII.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/bitcoincashII.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "bitcoincashII.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "bitcoincashII.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_bitcoincashII.conf"

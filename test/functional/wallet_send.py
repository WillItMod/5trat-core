...
class WalletSendTest(BitcoinTestFramework):
    ...
    def test_send(self, from_wallet, to_wallet=None, amount=None, data=None,
        ...
        if replaceable is None:
            replaceable = False # BCH2: no RBF support, always non-replaceable
        else:
        ...
        if add_to_wallet and not include_watching:
            ...
            tx = from_wallet.gettransaction(res["txid"])
            assert tx
            assert_equal(tx["bip125-replaceable"], "no")  # BCH2: no RBF, always "no"
    ...
    def run_test(self):
        ...
        for target, mode in product([-1, 0], ["bc2/kb", "sat/b"]):
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, change_address="not an address",
                       expect_error=(-5, "Change address must be a valid BCH2 address"))
        ...
        # BCH2: CashAddr format, P2PKH addresses start with "bitcoincashii:q"
        assert change_address.startswith("bitcoincashii:q"), f"Expected CashAddr P2PKH, got: {change_address}"
        ...
        # BCH2: RBF is not supported, bip125-replaceable is always "no"
        self.log.info("Replaceable (BCH2: no RBF, always non-replaceable)...")
        ...
        # BCH2: "External outputs" section skipped - uses sh(wsh(pkh())) SegWit descriptors
        # which are not supported in BCH2 (no SegWit/witness)
        self.log.info("External outputs (BCH2: skipped, requires SegWit descriptors)")

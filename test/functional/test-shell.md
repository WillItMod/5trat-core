...
```
>>> import sys
>>> sys.path.insert(0, "/path/to/bitcoinII/test/functional")
...
* `/path/to/bitcoinII/test/functional/combine_logs.py
...
|---|---|---|
| `bind_to_localhost_only` | `True` | Binds bitcoinIId RPC services to `127.0.0.1` if set to `True`.|
| `cachedir` | `"/path/to/bitcoinII/test/cache"` | Sets the bitcoinIId datadir directory. |
| `chain`  | `"regtest"` | Sets the chain-type for the underlying test bitcoinIId processes. |
| `configfile` | `"/path/to/bitcoinII/test/config.ini"` | Sets the location of the test framework config file. |

...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata, bool bch2_witness_via_scriptsig)
{
    ...
    if (solved && whichType == TxoutType::WITNESS_V0_KEYHASH)
    {
        ...
        TxoutType subType;
        SigVersion sigver = bch2_witness_via_scriptsig ? SigVersion::BCH_FORKID : SigVersion::WITNESS_V0;
        solved = solved && SignStep(provider, creator, witnessscript, result, subType, sigver, sigdata);
        if (!bch2_witness_via_scriptsig) {
    ...
    // Test solution — use BCH2 flags post-fork (includes opcode enablement)
    unsigned int verify_flags = bch2_witness_via_scriptsig ? BCH2_SCRIPT_VERIFY_FLAGS : STANDARD_SCRIPT_VERIFY_FLAGS;
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, bilingual_str>& input_errors)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        bool bch2_witness_via_scriptsig = (nHashType & SIGHASH_FORKID) != 0;
        // Only sign SIGHASH_SINGLE if there's a corresponding output:
        if (!fHashSingle || (i < mtx.vout.size())) {
            ProduceSignature(*keystore, MutableTransactionSignatureCreator(mtx, i, amount, &txdata, nHashType), prevPubKey, sigdata, bch2_witness_via_scriptsig);
        }

        UpdateInput(txin, sigdata);

        // amount must be specified for valid segwit signature (pre-fork only)
        if (!bch2_witness_via_scriptsig && amount == MAX_MONEY && !txin.scriptWitness.IsNull()) {
        ...
        if (bch2_witness_via_scriptsig) {
            // Post-fork: use BCH2 flags (requires FORKID, no WITNESS/TAPROOT)

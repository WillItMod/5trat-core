...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "bitcoincashII-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                ...
                // and spawn bitcoincashII-echo below instead of bitcoincashII-node. But
                // using bitcoincashII-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("bitcoincashII-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a bitcoincashIId
                // process not a bitcoincashIId-node process, so just create a local

...
void ParsePrevouts(const UniValue& prevTxsUnival, FillableSigningProvider* keystore, std::map<COutPoint, Coin>& coins)
{
    if (!prevTxsUnival.isNull()) {
        ...
        for (unsigned int idx = 0; idx < prevTxs.size(); ++idx) {
            ...
            // After BCH2 fork, new witness transactions are not valid, but existing PSBTs may still need processing.
...
void SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, const UniValue& hashType, UniValue& result, int nCurrentHeight)
{
    ...
    bool forkActive = (nCurrentHeight >= 0) ? consensusParams.IsBCH2ForkActive(nCurrentHeight + 1)
                                            : (consensusParams.BCH2ForkHeight != Consensus::NEVER_ACTIVE_HEIGHT);
    if (forkActive) {

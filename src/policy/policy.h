...
/** BCH2 post-fork minimum tx size: 65 bytes (Magnetic Anomaly reduced from 100 to mitigate
 *  CVE-2017-12842 while allowing smaller scripts like OP_TRUE spends). */
static constexpr unsigned int BCH2_MIN_STANDARD_TX_SIZE{65};
...
// After BCH2 fork, new witness outputs cannot be created.
...
static constexpr unsigned int BCH2_MANDATORY_SCRIPT_VERIFY_FLAGS{SCRIPT_VERIFY_P2SH |
                                                             SCRIPT_VERIFY_DERSIG |
                                                             SCRIPT_VERIFY_NULLDUMMY |
                                                             SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY |
                                                             SCRIPT_VERIFY_CHECKSEQUENCEVERIFY |
...
/**
 * No SegWit/Taproot, requires SIGHASH_FORKID, all BCH mandatory rules.
 * Includes opcode enablement flags — all BCH upgrades activate at the fork height,
 * and this flag set is only used when IsBCH2ForkActive() is true.
 */
static constexpr unsigned int BCH2_SCRIPT_VERIFY_FLAGS{SCRIPT_VERIFY_P2SH |
                                                         SCRIPT_VERIFY_DERSIG |
                                                         SCRIPT_VERIFY_NULLDUMMY |
                                                         SCRIPT_VERIFY_NULLFAIL |
                                                         SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY |

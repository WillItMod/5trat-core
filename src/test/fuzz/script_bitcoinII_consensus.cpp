...
// After BCH2 fork, SCRIPT_VERIFY_WITNESS is permanently disabled.
...
FUZZ_TARGET(script_bitcoinII_consensus)
{
    ...
    bitcoinIIconsensus_error* err_p = fuzzed_data_provider.ConsumeBool() ? &err : nullptr;
    const unsigned int n_in = fuzzed_data_provider.ConsumeIntegral<unsigned int>();
    const unsigned int flags = fuzzed_data_provider.ConsumeIntegral<unsigned int>();
    assert(bitcoinIIconsensus_version() == BITCOINIICONSENSUS_API_VER);
    if ((flags & SCRIPT_VERIFY_WITNESS) != 0 && (flags & SCRIPT_VERIFY_P2SH) == 0) {
        return;
    }
    (void)bitcoinIIconsensus_verify_script(random_bytes_1.data(), random_bytes_1.size(), random_bytes_2.data(), random_bytes_2.size(), n_in, flags, err_p);
    (void)bitcoinIIconsensus_verify_script_with_amount(random_bytes_1.data(), random_bytes_1.size(), money, random_bytes_2.data(), random_bytes_2.size(), n_in, flags, err_p);
    ...
    (void)bitcoinIIconsensus_verify_script_with_spent_outputs(

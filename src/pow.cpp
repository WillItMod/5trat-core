...
ProofTier ClassifyProofTier(const uint256& hash, unsigned int nBits) noexcept
{
    ...
    blue_target.SetCompact(nBits, &negative, &overflow);
    if (negative || overflow || blue_target == 0) return ProofTier::BLUE;

    const arith_uint256 proof{UintToArith256(hash)};
    if (proof <= blue_target / 12) return ProofTier::GOLD;
    if (proof <= blue_target / 4) return ProofTier::PINK;
    return ProofTier::BLUE;
}

const char* ProofTierName(ProofTier tier) noexcept
{
    switch (tier) {
    case ProofTier::GOLD:
        return "gold";
...
uint256 GetProofTierTarget(unsigned int nBits, ProofTier tier) noexcept
{
    bool negative{false};
    bool overflow{false};
    arith_uint256 target;
    target.SetCompact(nBits, &negative, &overflow);
    if (negative || overflow) return uint256{};
    if (tier == ProofTier::GOLD) target /= 12;
    if (tier == ProofTier::PINK) target /= 4;
...
{
    // The fresh 5TRAT genesis is a compiled bootstrap identity rather than a
    // spendable mined reward. Its exact hash is the sole exception to the
    // production powLimit; block one and every later header use normal PoW.

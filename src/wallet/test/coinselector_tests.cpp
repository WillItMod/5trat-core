...
// After BCH2 fork, BECH32 (SegWit) addresses are not available for new transactions.
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19.8 BC2 total
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 BC2 total
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            return available_coins;
        });
...
BOOST_AUTO_TEST_CASE(check_max_weight)
{
...

    {
        ...
        BOOST_CHECK(result);
        // Verify that only the 50 BC2 UTXO was selected
    ...
    {
        // Scenario 2:
        // to avoid exceeding transaction weight limits. Using 1 BC2 and 0.5 BC2 UTXOs instead.

        // The actor starts with 100x 1.0 BC2 and 100x 0.5 BC2 (150.0 BC2 total) unspent outputs
        // Then tries to spend 49.5 BC2

...
util::Result<CTxDestination> LegacyScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    if (LEGACY_OUTPUT_TYPES.count(type) == 0) {
        return util::Error{_("Error: Only \"legacy\" address type is supported after BCH2 fork")};
    }
...
void LegacyScriptPubKeyMan::LearnRelatedScripts(const CPubKey& key, OutputType type)
{
    ...
    if (type == OutputType::BECH32M) {
        LogPrintf("BCH2 Warning: BECH32M (Taproot) address type requested but not supported post-fork\n");
        return;
    }
...
std::unordered_set<CScript, SaltedSipHasher> LegacyScriptPubKeyMan::GetScriptPubKeys() const
{
    ...
    // It allows migrating wallets that were created before the BCH2 fork.
    for (const auto& script_pair : mapScripts) {
        ...
        if (IsMine(script) == ISMINE_SPENDABLE) {
            ...
            if (script.IsWitnessProgram(wit_ver, witprog)) {
                spks.insert(script);
                if (wit_ver == 1) {
                    LogPrintf("BCH2 Warning: Found Taproot (witness v1) script. Only key-path spending is supported post-fork.\n");
                } else if (wit_ver > 1) {
                    LogPrintf("BCH2 Warning: Found witness v%d script. This may not be spendable post-fork.\n", wit_ver);
                }
            }
        } else {
...
std::optional<MigrationData> LegacyScriptPubKeyMan::MigrateToDescriptor()
{
    ...
    // It allows migrating wallets that were created before the BCH2 fork with SegWit multisigs.
    for (const auto& script_pair : mapScripts) {
        const CScript script = script_pair.second;
...
bool DescriptorScriptPubKeyMan::SetupDescriptorGeneration(WalletBatch& batch, const CExtKey& master_key, OutputType addr_type, bool internal)
{
    ...
    // BCH2 uses coin type 145 (BCH standard) for mainnet, 1 for testnet/regtest

...
{
    // BCH2: RBF-based fee bumping is disabled after the fork. Use CPFP instead.
    const auto& params = Params().GetConsensus();
    int tipHeight = wallet.chain().getHeight().value_or(0);
    if (params.IsBCH2ForkActive(tipHeight + 1)) {
        errors.push_back(Untranslated("Fee bumping via RBF is disabled on BCH2. Use child-pays-for-parent (CPFP) instead."));

        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // Static vsize overhead + outputs vsize. 4 nVersion, 4 nLocktime, 1 input count (no witness overhead in BCH2)
    ...
    // BCH2: Force RBF signaling off post-fork — RBF is disabled, sequence should be MAX_SEQUENCE_NONFINAL
    bool signal_rbf = coin_control.m_signal_bip125_rbf.value_or(wallet.m_signal_rbf);
    if (signal_rbf && Params().GetConsensus().IsBCH2ForkActive(wallet.GetLastBlockHeight())) {
        signal_rbf = false;
    }

        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fRescan = true;
    {
        ...
        CKeyID vchAddress = pubkey.GetID();
        {
            ...
            // BCH2: Determine fork status to filter witness destinations
            int nHeight = pwallet->GetLastBlockHeight();
            const Consensus::Params& consensusParams = Params().GetConsensus();
            bool forkActive = consensusParams.IsBCH2ForkActive(nHeight);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            int nHeight = pwallet->GetLastBlockHeight();
            const Consensus::Params& consensusParams = Params().GetConsensus();
            if (consensusParams.IsBCH2ForkActive(nHeight) && IsWitnessDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Witness/SegWit addresses cannot be imported after BCH2 fork");
            }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fGood = true;
    {
        ...
        for (const auto& script_pair : scripts) {
            ...
            // BCH2: Skip importing witness scripts post-fork — they are unspendable
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
                ...
                "Note: This command is only compatible with legacy wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The BCH2 address for the private key"},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    file << strprintf("# * fork_id: BCH2 (SIGHASH_FORKID=0x40)\n");
    file << "\n";
...
static UniValue ProcessDescriptorImport(CWallet& wallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    try {
        ...
        bool bch2_fork_active = false;
        {
            LOCK(wallet.cs_wallet);
            int nHeight = wallet.GetLastBlockHeight();
            const Consensus::Params& consensusParams = Params().GetConsensus();
            bch2_fork_active = consensusParams.IsBCH2ForkActive(nHeight);
            if (bch2_fork_active) {
                ...
                if (output_type && *output_type != OutputType::LEGACY) {
                    ...
                    warnings.push_back("Importing SegWit descriptor for claiming existing UTXOs. "
                                       "New SegWit addresses cannot be generated after BCH2 fork.");
                }
            }
        }
...
RPCHelpMan importdescriptors()
{
    return RPCHelpMan{"importdescriptors",
            ...
            "\nBCH2 SEGWIT MIGRATION: After the BCH2 fork, SegWit descriptors (wpkh, wsh, tr) can be imported for claiming\n"
            ...
            "spending is supported. If you have Taproot script-path UTXOs, move them BEFORE the BCH2 fork activates.\n",
                {
                    {"requests", RPCArg::Type::ARR, RPCArg::Optional::NO, "Data to be imported",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",

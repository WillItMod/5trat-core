...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    if (m_last_block_processed_height >= 0 && consensusParams.IsBCH2ForkActive(m_last_block_processed_height + 1)) {
...
{
    LOCK(cs_wallet);
    WalletLogPrintf("CommitTransaction:\n%s", tx->ToString()); // NOLINT(bitcoinII-unterminated-logprintf)
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart bitcoincashIId with -walletcrosschain to override.");
                return false;
            }
        }
    }
...
void CWallet::SetupDescriptorScriptPubKeyMans(const CExtKey& master_key)
{
    ...
    // This is true regardless of block height since BCH2 is a new network.
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
        ...
        for (bool internal : {false, true}) {
            ...
            for (const UniValue& desc_val : descriptor_vals.get_array().getValues()) {
                ...
                // BCH2: Only accept legacy (pkh) descriptors from external signers.

...
// and legacy compatibility. After BCH2 fork, witness outputs cannot be created.
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoincashII-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  bitcoincashII-tx [options] <hex-tx> [commands]  Update hex-encoded BCH2 transaction\n"
                "or:     bitcoincashII-tx [options] -create [commands]   Create hex-encoded BCH2 transaction\n"
                "\n";

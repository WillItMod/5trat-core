...
struct Params {
    ...
    /** Initial block subsidy in atomic units. Production 5TRAT uses 5 coins;
    ...
    /**
     * Fresh-chain launch mode. Block one keeps the compiled anchor target, then
     * ASERT measures elapsed time from block one's accepted timestamp. This
     * prevents time spent waiting for the first miner from lowering difficulty.
     */
    bool fASERTAnchorAtFirstBlockTime{false};

    /** ASERT half-life constants */
    static constexpr int64_t ASERT_HALFLIFE_1_HOUR = 60 * 60;        // 3600 seconds
    static constexpr int64_t ASERT_HALFLIFE_30_MINUTES = 30 * 60;    // 1800 seconds

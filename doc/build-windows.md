...
Below are some notes on how to build Bitcoin Cash II Core for Windows.

The options known to work for building Bitcoin Cash II Core on Windows are:
...
Note that for WSL the Bitcoin Cash II Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoincashII-core, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoincashII`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoincashII

UNIX BUILD NOTES
====================
Some notes on how to build Bitcoin Cash II Core in Unix.
...
memory available when compiling Bitcoin Cash II Core. On systems with less, gcc can be
...
To build Bitcoin Cash II Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build bitcoincashII-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a bitcoincashII-qt executable will be
built by default.
...
...
to: /path/to/bitcoincashII-core/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/bitcoincashII-core/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Bitcoin Cash II Core can

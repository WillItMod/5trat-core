...
AC_DEFUN([BITCOINII_QT_FAIL],[
  if test "$bitcoincashII_qt_want_version" = "auto" && test "$bitcoincashII_qt_force" != "yes"; then
    if test "$bitcoinII_enable_qt" != "no"; then
      AC_MSG_WARN([$1; bitcoincashII-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOINII_QT_CHECK],[
  if test "$bitcoinII_enable_qt" != "no" && test "$bitcoincashII_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOINII_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build bitcoincashII-qt GUI (default=auto)])],
    [
     bitcoincashII_qt_want_version=$withval
     if test "$bitcoincashII_qt_want_version" = "yes"; then
       bitcoincashII_qt_force=yes
       bitcoincashII_qt_want_version=auto
     fi
    ],
    [bitcoincashII_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); bitcoincashII_qt_want_version=qt5],
        [qt_lib_suffix= ])
